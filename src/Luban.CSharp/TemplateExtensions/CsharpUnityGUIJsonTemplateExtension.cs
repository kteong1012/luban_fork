...
public class CsharpUnityGUIJsonTemplateExtension : ScriptObject
{
    ...
    public static string RenderField(DefField field, string name)
    {
        var ttype = field.CType;
        var fieldName = TypeTemplateExtension.FormatFieldName(CodeFormatManager.Ins.CsharpDefaultCodeStyle, field.Name);
        return ttype.Apply(UnityGUIRender.Ins, $"{name}.{fieldName}", 0);
    }
    ...
    public static bool NeedSerializeReference(TType type)
    {
        if (type is TBean bean)
        {
            return bean.DefBean.IsAbstractType;
        }
        if (type is TList list)
        {
            return NeedSerializeReference(list.ElementType);
        }
        if (type is TArray array)
        {
            return NeedSerializeReference(array.ElementType);
        }
        if (type is TMap map)
        {
            return NeedSerializeReference(map.ValueType) || NeedSerializeReference(map.KeyType);
        }
        if (type is TSet set)
        {
            return NeedSerializeReference(set.ElementType);
        }
        return false;
    }
}
