    ...
    public string Accept(TBean type, string fieldName, int depth)
    {
        ...
        else
        {
            if (type.DefBean.HierarchyExportFields.Count > 0)
            {
                ...
                var sb = $$"""
                {
                    UnityEditor.EditorGUILayout.BeginVertical(GUI.skin.button);
                    global :: {{type.DefBean.FullNameWithTopModule}}.Render{{type.DefBean.Name}}({{fieldName}});
    ...
    public string Accept(TMap type, string fieldName, int depth)
    {
        ...
        return $$"""
        {
            ...
            for (int {{__i}} = 0; {{__i}} < {{__n}}; {{__i}}++)
            {
                ...
                if ({{__e}} == null)
                {
                    {{__e}} = new() { key = {{__key}},  value = {{__value}} };
                    {{fieldName}}[{{__i}}] = {{__e}};
                }
                else
                {
