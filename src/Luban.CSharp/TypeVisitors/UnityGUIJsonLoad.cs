...
class UnityGUIJsonLoad : ITypeFuncVisitor<string, string, int, string>
{
    ...
    public string Accept(TString type, string json, string x, int depth)
    {
        if (CsharpUnityGUIJsonTemplateExtension.IsUnityObjectFieldType(type))
        {
            ...
            return $$"""
                ...
                {{x}} = EditorConfigUtil.GetObj<{{objTypeName}}>({{jsonVar}});
                """;
        }
        else
        {
    ...
    public string Accept(TMap type, string json, string x, int depth)
    {
        ...
        var __k = $"__k{depth}";
        var __v = $"__v{depth}";
        var ret = @$"if(!{json}.IsArray) {{ throw new SerializationException(); }} {x} = new {type.Apply(UnityGUIDeclaringTypeNameVisitor.Ins)}(); foreach(SimpleJSON.JSONNode {__e} in {json}.Children) {{ {type.KeyType.Apply(UnityGUIDeclaringTypeNameVisitor.Ins)} {__k};  {type.KeyType.Apply(this, $"{__e}[0]", __k, depth + 1)} {type.ValueType.Apply(UnityGUIDeclaringTypeNameVisitor.Ins)} {__v};  {type.ValueType.Apply(this, $"{__e}[1]", __v, depth + 1)}  {x}.Add(new() {{ key = {__k}, value = {__v} }}); }}  ";
        return ret;
    }
